...
- 自动隐藏 island 默认忽略鼠标事件；悬停唤出或点击 pin/settings 时临时接管鼠标，避免平时挡住菜单栏交互；刘海屏也使用同一套 pin/unpin 与 auto-hide 语义，只保留 notch-aware 的紧凑视觉布局。
