...
| 测试 | B | 覆盖了显示格式、Codex usage response 动态窗口、本地 token 扫描、跨天日级聚合、指定日期小时级模型聚合、长期 session 旧目录补捞、缓存失效、Usage 视图级缓存复用和 fallback scanner smoke path；CI 已运行 `swift test`。 | 补更完整的 UI 刷新节奏测试。 |
