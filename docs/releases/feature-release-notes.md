...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-05-11 | 多屏体验 | 14 寸等内建刘海屏也支持右侧 pin/unpin；取消 pin 后同样会自动收起，悬停屏幕顶边再唤出。 | 刘海屏保留 notch-aware 紧凑 island 和物理 notch 中心禁区，但不再作为 auto-hide/pin 的例外；pin 偏好继续复用 `AgentBar.pinnedOpen`。 |
