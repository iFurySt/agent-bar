...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-04-27 | 多账号管理 | 用户保存超过 4 个 Codex 账号后，顶部展开区不会再静默隐藏后续账号，而是能明确进入完整账号列表查看和切换。 | island 展开区继续保持当前账号加最近账号的轻量 4 行布局，超过 4 个账号时显示 `View N more accounts` 入口；设置窗口新增 Accounts 页面，集中展示所有已见账号、5h/7d quota、当前状态和切换按钮。 |
