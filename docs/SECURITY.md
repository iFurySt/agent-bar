...
- Claude Code 配额只读取 `~/.claude/.credentials.json`，不读取 macOS Keychain；本机没有这个文件（例如 Claude Code 把凭据存在 Keychain 里）时视为没有可用 Claude Code 凭据并隐藏卡片，避免后台轮询触发系统授权弹窗。

## 网络

- 仅为获取实时 5h/7d quota 调用 `https://chatgpt.com/backend-api/wham/usage`。
- 仅在本机 `~/.claude/.credentials.json` 存在时，调用 Anthropic OAuth usage API 读取 Claude Code 5h/weekly quota。
