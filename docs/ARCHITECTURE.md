...
1. `CodexRateLimitFallbackScanner` 优先扫描最近 session 里的 `event_msg.token_count.rate_limits`，取基础 `limit_id == "codex"` 的 primary/secondary window，把 used percent 换算为 remaining percent，并保留 `window_minutes` 与 `resets_at`。Codex 窗口不再假定固定为 5h/7d：已知时长显示为 5h、Daily、7d、Monthly 或 Annual，未知时长使用通用标签。
...
6. `AgentBarDisplayFormatting` 输出单行文案，`IslandWindowController` 为每块 `NSScreen` 建立 overlay，并每 60 秒静默刷新一次；刷新过程不显示状态点，先用 quick quota 更新顶部百分比，如果展开区账号 quota 仍为空则先单独拉取账号 usage，再等待完整 cost 扫描完成后写回缓存并更新文案。顶部只渲染有有效百分比的 Codex 窗口，按周期从短到长排列；缺失窗口不保留 `--%`。数字变化由 `RollingTextLabel` 做短时向上滚动，位数变化带来的 island 宽度变化用 0.44s ease-in-out 过渡，并遵守系统 Reduce Motion。所有屏幕都使用贴住顶部、水平居中的黑色 island shell，高度优先采用 Window Server 暴露的当前屏 `Menubar` 窗口并归一化为视觉菜单栏高度，再回退到该屏 `frame.maxY - visibleFrame.maxY`、`safeAreaInsets.top` 和物理 notch 高度推导；这样普通桌面、全屏 app 顶部 hover 下拉菜单栏、刘海屏和无 notch 屏都保持与系统顶栏同高。刘海屏通过 `safeAreaInsets` 与 `auxiliaryTopLeftArea`/`auxiliaryTopRightArea` 计算物理 notch，把 notch 本体作为中心禁区，只显示当前可用 quota 的百分比，hover 或展开后才显示右侧设置入口；普通屏幕保留相同视觉语言，但用连续 island 显示带周期标签的完整信息。账号展开区在鼠标离开面板后开始计时，默认连续离开 200ms 自动收回；设置窗口可调整 100 到 5000ms，鼠标回到面板内会重新计算。
...
10. `ClaudeUsageClient` 只读取 `~/.claude/.credentials.json`（`ClaudeAuthStore`）；本机没有这个文件（比如
    Claude Code 把凭据存在 macOS Keychain 里）时，`claudeRateLimits` 直接视为不可用并隐藏 Claude Code 卡片，
    不再尝试读 Keychain。之前用 `kSecUseAuthenticationUI`/`LocalAuthentication` 做过 non-interactive Keychain
    读取，但这类 API 并不能在所有系统上都可靠地避免弹出系统授权窗口；Claude Code 配额卡片本身也没有“用户主动
    刷新”这种可以把交互式访问限定在其后的操作入口，所以直接去掉了 Keychain 读写路径，彻底避免后台轮询触发
    授权弹窗。`ClaudeAuthStore` 在内存里缓存已解析的凭据最多 30 分钟，靠文件 mtime/size 指纹判断是否需要重新
    读盘；60 秒一次的后台轮询因此不会每次都重新读文件，只有指纹变化、缓存过期或 token 刷新后写回时才会真正
    读盘。`accessToken` 临近过期或请求失败时用 `ClaudeTokenRefresher` 调用
