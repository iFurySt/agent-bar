...
- 为 Sparkle appcast 生成 EdDSA 更新签名，客户端只接受与内置公钥匹配的 release DMG。
...
- `scripts/generate-sparkle-appcast.sh`：用 `AGENT_BAR_SPARKLE_PRIVATE_KEY` 为 GitHub Release DMG 生成 Sparkle appcast 签名。
...
- OSV 的效果依赖仓库里存在可识别的依赖清单或 lockfile。
- 只有当 `scripts/release-package.sh` 真的代表项目的构建产物时，provenance 才真正有意义。
- Sparkle 私钥只能保存在 GitHub Actions secret 或受控本机 Keychain 中，不能提交到仓库；如果需要轮换，需要同时更新客户端内置公钥并发布一个过渡版本。
